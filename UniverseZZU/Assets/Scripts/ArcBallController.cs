...
/// TODO: Add inertia (free spin on mouse release)
/// TODO: Add rotation wrap around (continous rotation also after edge of arball is passed)
/// TODO: Add range constraint (fixed axis constraint is implemented). Allowing for limited rotation around non constrained axis (maybe using Dot product)
/// FIX: Orthographic camera + arcball not centered -> seems to produce strange rotations, check it.
...
public class ArcBallController : MonoBehaviour {
    ...
    */
	...
	private void InitArcball(){
		...
		if (GetComponent<Collider>().Raycast (ray, out hit, raycastDist)) { //called from OnMouseDown, so we always hit
